...
void loop(void)
{
  ...
  if (aa == 0) { //スタートするまでのステート <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    if (a == 0) //ステート0 キャリブレーション スイッチが押されてない時に押されるまで待つ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    {
      ...
      if (digitalRead(SWICH) == LOW) { //スイッチが押されたら
    ...
    else if (a == 1) //ボタンが指で離されたのを確認したら2回目のスイッチが押されるのを待つステートに行く
    {
      ...
      if (digitalRead(SWICH) == LOW) { //0.1秒経ってもスイッチが押されていたらチャタリングではないと判断
        a = 4;  //スイッチが離されるのを待つ
      } else {
        a = 0;  //チャタリングしていたため、リセット
      };
    }
    else if (a == 4) { //二回目のスイッチを押されるのを待つために一回目の指を離したのを検知する
      if (a != b) //初期化
      {
        b = a;
      };

      if (digitalRead(SWICH) == HIGH) { //指がスイッチから離れたら
        delay(100);
        a = 5;
      }
    }
      ...
      if (digitalRead(SWICH) == HIGH) { //チャタリングしていなかったら
        a = 6;
      }
      else
      {
      ...
      //Dirの設定
      DIR20 = DIR0 + 20;    //20゜
      if (360 < DIR20) { //角度が360を超えている数値になってしまったら
        DIR20 = DIR20 - 360;
      };
      DIR90 = DIR0 + 90;    //90゜
      if (360 < DIR90) { //角度が360を超えている数値になってしまったら
        DIR90 = DIR90 - 360;
      };
      DIR180 = DIR0 + 180;  //180°
      if (360 < DIR180) { //角度が360を超えている数値になってしまったら
        DIR180 = DIR180 - 360;
      };
      DIR270 = DIR0 + 270;    //270゜
      if (360 < DIR270) { //角度が360を超えている数値になってしまったら
        DIR270 = DIR270 - 360;
      };
      DIR340 = DIR0  + 340;   //340°
      if (360 < DIR340) { //角度が360を超えている数値になってしまったら
        DIR340 = DIR340 - 360;
      };
  ...
  {
    //ロボットを速く動かす（ボールを追う、ラインから離れる）時のモーターの設定値  このステートのところでこのパワーを変更したかったら条件のところに書けば書き換えられる
    if (i == 0) { //ラインが動作するステートにいるときに反応していなかったら
      if (analogRead(LINE13) > LineValue) //前にラインがある
      {
        if(g == 0)  //ボールを追いかける動作を前回していたら
        {
          if(j != 1){  //前回と同じブロックのラインが反応していたら前の動作を追加で続行する
            h = 50;  //ラインから離れる（前の動作の逆転をする）
          }
          else
          {
            h = 60;  //前回動作と逆の方向に動く
      ...
      else if (analogRead(LINE12) > LineValue) //前にラインがある
      {
        ...
        LineResetT = millis();
        LineResetT += 1000;  //ラインがあったらタイマースタート
      ...
      else if (analogRead(LINE23) > LineValue) //左にラインがある
      {
        if(g == 0)  //ボールを追いかける動作を前回していたら
        {
          if(j != 2){  //前回と同じブロックのラインが反応していたら前の動作を追加で続行する
            h = 75;  //右進（ラインが左にあったため）
          }
          else
          {
      ...
      else if (analogRead(LINE33) > LineValue) //後ろにラインがある
      {
        if(g == 0)  //ボールを追いかける動作を前回していたら
        {
          if(j != 3){  //前回と同じブロックのラインが反応していたら前の動作を追加で続行する
            h = 50;  //ラインから離れる（前の動作の逆転をする）
          }
          else
          {
      ...
      else if (analogRead(LINE43) > LineValue) //右にラインがある
      {
        if(g == 0)  //ボールを追いかける動作を前回していたら
        {
          if(j != 4){  //前回と同じブロックのラインが反応していたら前の動作を追加で続行する
            h = 85;  //左進（ラインが右にあったため）
          }
          else
          {
    ...
    else  //ラインから離れるっていう動作をしていなかったらラインが反応しているので一旦ストップ
    {
      if (i == 1) { //ラインセンサーが反応したら一旦止まる(100経つとラインから離れる動きに変わる)
        //ストップ
        digitalWrite(INA1, LOW);
        digitalWrite(INB1, LOW); //停止
    ...
    if (Motor1 == 0) { //もし停止しろ（0）と言われたら停止
      digitalWrite(INA1, LOW);
      digitalWrite(INB1, LOW); //停止
      m = 0;  //変換していないというフラグ
    };
    if (Motor1 > 250) { //もし255を超えると、値が一からになってしまうので、ならないようにするプログラム
      Motor1 = 250;
    };
    analogWrite(PWM1, Motor1); //右前モータ
    ...
    if (Motor2 == 0) { //もし停止しろ（0）と言われたら停止
      digitalWrite(INA2, LOW);
      digitalWrite(INB2, LOW); //停止
      m = 0;  //変換していないというフラグ
    };
    if (Motor2 > 250) { //もし255を超えると、値が一からになってしまうので、ならないようにするプログラム
      Motor2 = 250;
    };
    ...
    if (Motor3 == 0) { //もし停止しろ（0）と言われたら停止
      digitalWrite(INA3, LOW);
      digitalWrite(INB3, LOW); //停止
      m = 0;  //変換していないというフラグ
    };
    if (Motor3 > 250) { //もし255を超えると、値が一からになってしまうので、ならないようにするプログラム
      Motor3 = 250;
    };
    ...
    if (Motor4 == 0) { //もし停止しろ（0）と言われたら停止
      digitalWrite(INA4, LOW);
      digitalWrite(INB4, LOW); //停止
      m = 0;  //変換していないというフラグ
    };
    if (Motor4 > 250) { //もし255を超えると、値が一からになってしまうので、ならないようにするプログラム
      Motor4 = 250;
    };
    ...
    else if(a == 60)  //前回と同じ動作をするため、モーターの式は不要なため、このステートはただのスキップのステート
    {
      if (a != b) //初期化
      {
        b = a;
